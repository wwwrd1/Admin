...
def api_getOnline(request):
    ...
    try:
        ...
        # 网格视图只显示处理后的视频流，列表视图显示所有流
        if view_type == "grid":
            mediaServerState, data = __getAllOnlineStream(is_filter_analyzer=True, only_analyzer=True)
        else:
...
def __getAllOnlineStream(is_filter_analyzer=False, only_analyzer=False):
    data = []
    ...
    if mediaServerState:
        ...
        for online_stream in online_data:
            ...
            # 修改判断逻辑
            if only_analyzer:
                # 九宫格视图：只显示 analyzer 开头的流
                if app.startswith('analyzer'):
                    db_stream = db_stream_dict.get(app_name, None)
                    if db_stream:
                        online_stream["source_type"] = 1
                        online_stream["source"] = db_stream
                        online_stream["source_nickname"] = db_stream["nickname"]
                    else:
                        online_stream["source_type"] = 0
                        online_stream["source_nickname"] = "{app}/{name}".format(app=app, name=name)
                    data.append(online_stream)
            elif is_filter_analyzer:
                # 列表视图：显示所有流，但要过滤掉一些特殊流
                if app == "live" or app.startswith('analyzer'):
                    db_stream = db_stream_dict.get(app_name, None)
                    if db_stream:
                        online_stream["source_type"] = 1
                        online_stream["source"] = db_stream
